...
锁定行为:
- socketio disconnect 必须清理 clientIds (避免泄漏 + 旧 sid 残留)
- clientIds 是 instance variable, 不是 class variable
- 同一 clientId 重复 connect, 旧 sid 必须让位 (不能发到错误 sid)
"""
import pytest
...
# --------------------------------------------------------------------------- #
# clientIds 是 instance variable
# --------------------------------------------------------------------------- #
@pytest.mark.unit
def test_clientids_is_instance_variable():
    """clientIds 不能是 class variable, 否则跨实例共享。"""
...
# --------------------------------------------------------------------------- #
# connect 记录反向索引, disconnect 时清理
# --------------------------------------------------------------------------- #
@pytest.mark.unit
@pytest.mark.asyncio
async def test_connect_records_reverse_index():
    """connect 时除写 clientIds, 还写 _sid_to_client 反向索引。"""
...
async def test_disconnect_cleans_clientids():
    """disconnect 必须把 clientId 从 clientIds 里删掉, 防泄漏。"""
    ...
    assert "user_B" not in async_io.clientIds, (
        "disconnect 后 clientId 仍残留在 clientIds, 内存泄漏"
    )
...
async def test_disconnect_does_not_remove_other_clients():
    """disconnect 一个 sid 不该误删别的 clientId。"""
    from app.ws.io import async_io, api_namespace_connect, api_namespace_disconnect
    async_io.clientIds.clear()
    async_io._sid_to_client.clear()
...
# --------------------------------------------------------------------------- #
# 同一 clientId 重复 connect 旧 sid 让位
# --------------------------------------------------------------------------- #
@pytest.mark.unit
@pytest.mark.asyncio
async def test_duplicate_connect_evicts_old_sid():
    """同一 clientId 二次 connect, 旧 sid 必须从 _sid_to_client 移除。"""
    from app.ws.io import async_io, api_namespace_connect
    async_io.clientIds.clear()
    async_io._sid_to_client.clear()
