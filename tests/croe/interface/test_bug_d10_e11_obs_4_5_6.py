...
def test_bug_d10_update_by_id_accepts_post_update_hook():
    """[BUG-D10] update_by_id 签名必须有 post_update_hook 参数。"""
...
def test_bug_d10_update_cls_keeps_target_attached_for_hook():
    """[BUG-D10] update_by_id 必须在 expunge 之前调 post_update_hook (target 还 attached)。"""
    from app.mapper import Mapper
    sig_src = inspect.getsource(Mapper.update_by_id)
    ...
    assert update_cls_call_pos > 0, (
        f"[BUG-D10] update_by_id 必须调 update_cls, 当前 src: {sig_src[:600]}"
    )
    assert hook_call_pos > 0, (
        f"[BUG-D10] update_by_id 必须 None check hook, 当前 src: {sig_src[:600]}"
    )
    assert hook_call_pos > update_cls_call_pos, (
        f"[BUG-D10] post_update_hook 必须在 update_cls 之后调 (target 还 attached)。 "
...
async def test_bug_d10_no_hook_returns_target_as_before():
    """[BUG-D10] 不传 post_update_hook 时, update_by_id 仍返 target (向后兼容)。"""
    from app.mapper import Mapper
...
def test_bug_d10_update_interface_case_uses_post_update_hook():
    ...
    assert "post_update_hook" in src, (
        f"[BUG-D10] update_interface_case 必须用 post_update_hook, 当前 src:\n{src[:500]}"
    )
    # 不应在 update_by_id 之后还有 to_dict() 调用 (那是 bug)
    # 简化检查: `new_case.to_dict()` 不应再出现
    assert "new_case.to_dict()" not in src, (
        f"[BUG-D10] update_interface_case 不应在 update_by_id 后调 to_dict(), "
...
def test_bug_d10_update_interface_uses_post_update_hook():
    ...
    assert "post_update_hook" in src, (
        f"[BUG-D10] update_interface 必须用 post_update_hook"
    )
...
def test_bug_e11_condition_step_writes_assert_data():
    """[BUG-E11] step_content_condition 写库时必须含 assert_data=content_condition。"""
...
def test_bug_obs_4_starter_msg_type_constant_exists():
    """[BUG-OBS-4] starter 模块必须有 STARTER_MSG_TYPE 常量 + 5+ 类型。"""
...
def test_bug_obs_4_starter_has_send_typed_method():
    ...
    assert hasattr(APIStarter, "send_typed"), (
        f"[BUG-OBS-4] APIStarter 缺 send_typed 方法, 推荐用结构化 [TYPE_xxx] 前缀"
    )
...
async def test_bug_obs_4_send_typed_prepends_type_marker():
    """[BUG-OBS-4] send_typed 调用 super().send 时, msg 必须含 [TYPE_xxx] 前缀。"""
...
def test_bug_obs_5_set_result_field_returns_bool():
    """[BUG-OBS-5] InterfaceCaseResultMapper.set_result_field 源码必须返 bool。"""
    ...
    # 必须有 `-> bool` 返回类型注解
    assert "-> bool" in src, (
        f"[BUG-OBS-5] set_result_field 应 -> bool, 当前:\n{src}"
    )
    # 必须有 return True
    assert "return True" in src, (
        f"[BUG-OBS-5] set_result_field 成功路径必须 return True, 当前:\n{src}"
    )
...
def test_bug_obs_6_runner_logs_case_result_id():
    """[BUG-OBS-6] run_interface_case 第一条 log 后必须显式 log case_result_id。"""
...
async def test_bug_obs_6_log_message_contains_case_result_id():
    """[BUG-OBS-6] 端到端: log.info 触发后, captured log message 必须含 case_result_id=。"""
