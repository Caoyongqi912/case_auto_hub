"""app/controller/ 全部用 bare raise + log.exception, 保留 traceback.
...
def test_no_raise_e_in_controller(controller_file: Path):
    """controller 的 except 块内不能用 `raise e`, 必须 bare raise 防 shadow."""
    ...
    assert not bad, (
        f"{controller_file}: {len(bad)} 处 `raise e` 应改为 bare `raise`"
...
def test_no_log_error_in_except(controller_file: Path):
    """controller 的 except 块内不能用 `log.error`, 必须用 `log.exception` 保留 traceback."""
    text = controller_file.read_text()
    lines = text.splitlines()
    blocks = _collect_except_blocks(lines)
    bad: list[tuple[int, str]] = []
    for start, end in blocks:
        for ln in range(start - 1, end):
            if re.match(r"^\s*log\.error\(", lines[ln]):
                bad.append((ln + 1, lines[ln].rstrip()))
    assert not bad, (
        f"{controller_file}: {len(bad)} 处 except 块内 `log.error` 应改为 `log.exception`"
...
def test_interfaceController_export_yaml_uses_log_exception_and_bare_raise():
    """回归 interfaceController 导出 YAML: log.exception + bare raise."""
...
def test_case_plan_delete_permanent_uses_log_exception():
    """回归 case_plan.delete_permanent: log.exception 而非 log.error."""
    from pathlib import Path as _P
