...
class PlayLocator(BaseModel):
    """

    """
    __tablename__ = "play_locator"
    getter_name = Column(String(255), comment="定位器")
    getter_desc = Column(String(255), nullable=True, comment="定位器描述")
    getter_demo = Column(String(255), nullable=True, comment="定位器示例")

    def __repr__(self):
        return (
            f"<PlayLocator (getter_name='{self.getter_name}')>"
        )
...
class PlayMethod(BaseModel):
    ...
    need_key = Column(INTEGER, nullable=False, comment="需要键 1 是 2否")
