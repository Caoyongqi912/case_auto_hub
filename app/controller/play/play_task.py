...
async def insert_task(taskInfo: InsertPlayTaskSchema, user: User = Depends(Authentication())):
    """
    获取任务信息
    :param taskInfo:
    :param _:user
    :return:
    """
    data = await PlayTaskMapper.save(creator_user=user, **taskInfo.model_dump())
...
@router.post("/updateTask", description="更新任务信息")
async def update_task(task: UpdatePlayTaskSchema, user: User = Depends(Authentication())):
    """
    获取任务信息
    :param task:
    :param user:user
    :return:
    """
    data = await PlayTaskMapper.update_by_id(**task.model_dump(
        exclude_none=True,
        exclude_unset=True
    ), update_user=user)
    return Response.success(data)
