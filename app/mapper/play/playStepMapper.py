...
from app.model.playUI import PlayStep, PlayCaseStepAssociation, PlayCase, PlayStepModel
...
class PlayStepMapper(Mapper[PlayStep]):
    __model__ = PlayStep
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    ...
                    await cls.save(session=session, **kwargs)
        except Exception as e:
            raise e
        ...
        try:
            async with async_session() as session:
                async with session.begin():
                    ...
                    await cls.save(**kwargs, session=session)
