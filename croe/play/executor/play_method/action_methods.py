...
from .result_types import InfoDict, create_error_info, create_success_result, create_error_result
...
class ClickMethod(BaseMethods):
    ...
    async def execute(self, context: StepContext, locator: Optional[Locator] = None) -> tuple[bool, Optional[InfoDict]]:
        try:
            ...
            result = create_success_result(message=f"点击元素成功: {context.selector}")
...
class EvaluateMethod(BaseMethods):
    ...
    async def execute(self, context: StepContext, locator: Optional[Locator] = None) -> tuple[bool, Optional[InfoDict]]:
        try:
            ...
            result_obj = create_success_result(
