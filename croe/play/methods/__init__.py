...
@dataclass
class PlayStepHandler:
    page: Page
    play_step: PlayStep
    starter: UIStarter
    vars_tarns: VariableTrans
...
class _MethodHandler(ABC):
    ...
    @abstractmethod
    async def handle(self, play: PlayStepHandler):
        pass
...
class GetAttrHandler(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        ...
        try:
            try:
                key, attr_name = play.play_step.fill_value.split(";")
            except Exception:
                await play.starter.send(f"提取变量格式错误  ⚠️ :{play.play_step.fill_value}")
                raise ValueError(f"提取变量格式错误 ⚠️ :{play.play_step.fill_value}")
            locator = await get_locator(play.page, play.play_step)
            attr = await locator.get_attribute(attr_name)
            if attr:
                tempVariable = {
                    key: attr.strip()
                }
                await play.starter.send(f"提取变量 ✅ {tempVariable}")
                await play.vars_tarns.add_var(key=key, value=attr.strip())
                await Writer.write_vars_info(case_result=play.case_result,
                                             extract_method=play.play_step.method,
                                             step_name=play.play_step.name,
                                             varsInfo=tempVariable)
        except Exception:
            raise
...
class GetTextHandler(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        ...
        try:
            ...
            tempVariable = {
                play.play_step.fill_value.strip(): text.strip()
            }
            await play.starter.send(f"提取变量 ✅ {tempVariable}")
            await play.vars_tarns.add_var(key=play.play_step.fill_value.strip(), value=text.strip())
            await Writer.write_vars_info(case_result=play.case_result,
                                         extract_method=play.play_step.method,
                                         step_name=play.play_step.name,
                                         varsInfo=tempVariable)
...
class GOTOHandler(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        ...
        try:
            ...
            url = await play.vars_tarns.trans(play.play_step.fill_value)
            await play.starter.send(f"执行 >> 打开地址:\"{url}\"")
            await play.page.goto(url, wait_until="load")
...
class WaitHandler(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        try:
            await play.page.wait_for_timeout(float(play.play_step.fill_value))
            await play.starter.send(f"wait_for_timeout  {play.play_step.fill_value} done")
        except Exception as e:
            raise e
...
class EvaluateHandler(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        try:
            ...
            result = await play.page.evaluate(play.play_step.fill_value)
            await play.starter.send(f">> evaluate:  \"{play.play_step.fill_value}\" result: {result}")
        except Exception as e:
            raise e
...
class FillHandler(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        try:
            locator = await get_locator(play.page, play.play_step)
            fill_value = await play.vars_tarns.trans(play.play_step.fill_value)
            await play.starter.send(f"输入 >> \"{fill_value}\"")
...
class PlayClickHandler(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        try:
            locator = await get_locator(play.page, play.play_step)
            await locator.click()
            await play.starter.send(f"点击 {locator} ✅")
        except Exception as e:
            raise e
...
class PlayDBClickHandler(_MethodHandler):
    ...
    async def handle(self,play: PlayStepHandler):
        try:
            ...
            await play.starter.send(f"双击 {locator} ✅")
        except Exception as e:
            raise e
...
class PlayPressKeyHandler(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        try:
            await play.starter.send(f"use keyboard press \"{play.play_step.fill_value.strip()}\"")
            await play.page.keyboard.press(play.play_step.fill_value.strip())
        except Exception as e:
            raise e
...
class PlayAssertToHaveText(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        assert_info = set_assert_info(play_step=play.play_step, opt="=", expect_value=play.play_step.fill_value)
        try:
            locator = await get_locator(play.page, play.play_step)
            await expect(locator).to_have_text(play.play_step.fill_value,
                                               use_inner_text=True)
            ...
            await Writer.write_assert_info(play.case_result, [assert_info])
...
class PlayAssertToBeAttachedAndVisible(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        assert_info = set_assert_info(play_step=play.play_step, opt="=", expect_value=True)
        try:
            locator = await get_locator(play.page, play.play_step)
...
class PlayAssertNotToBeAttachedAndVisible(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        assert_info = set_assert_info(play_step=play.play_step, opt="!=", expect_value=False)
        try:
            locator = await get_locator(play.page, play.play_step)
...
class PlayAssertToHaveAttribute(_MethodHandler):
    ...
    async def handle(self, play:PlayStepHandler):
        key, attr = play.play_step.fill_value.split(":")
        assert_info = set_assert_info(play_step=play.play_step, opt="=", expect_value=attr)
        try:
            locator = await get_locator(play.page, play.play_step)
...
class PlayAssertToHaveTitle(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        ...
        assert_info = set_assert_info(play_step=play.play_step, opt="=", expect_value=play.play_step.fill_value)
        try:
            await expect(play.page).to_have_title(play.play_step.fill_value)
            assert_info["assert_actual"] = await play.page.title()
            assert_info["assert_result"] = True
        except AssertionError as e:
            assert_info["assert_actual"] = await get_error_value(e)
            raise
...
class PlayAssertToHaveURL(_MethodHandler):
    ...
    async def handle(self, play: PlayStepHandler):
        ...
        assert_info = set_assert_info(play_step=play.play_step, opt="=", expect_value=play.play_step.fill_value)
        try:
            await expect(play.page).to_have_url(play.play_step.fill_value)
            assert_info["assert_actual"] = play.page.url
            assert_info["assert_result"] = True
        except AssertionError as e:
            assert_info["assert_actual"] = await get_error_value(e)
            raise
...
class CustomizeMethod:
    ...
    @classmethod
    async def play(cls, page: Page, play_step: PlayStep, starter: UIStarter, vars_tarns: VariableTrans,
                   case_result: PlayCaseResult = None):
        """执行方法"""
        ...
        play = PlayStepHandler(page=page, play_step=play_step, starter=starter, vars_tarns=vars_tarns, case_result=case_result)
...
async def new_page(page: Page, play_step: PlayStep, starter: UIStarter, vars_tarns: VariableTrans) -> Page:
    """
    actions On New Page
